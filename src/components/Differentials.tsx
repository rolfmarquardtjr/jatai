...
const Differentials = () => {
  ...
  return (
    <section className="py-20 bg-white" id="diferenciais" ref={elementsRef}>
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-jatai-dark-purple mb-4 reveal">Nossos Diferenciais</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
          {differentials.map((item, index) => (
            <div 
              key={index} 
              className="bg-gradient-to-br from-gray-50 to-white p-6 rounded-2xl shadow-lg flex gap-4 reveal"
              style={{ animationDelay: `${index * 0.2}s` }}
            >
              <div className="bg-jatai-purple rounded-full p-1 h-fit mt-1">
                <Check className="w-5 h-5 text-white" />
              </div>
              <div>
                <h3 className="text-xl font-semibold text-jatai-dark-purple mb-2">{item.title}</h3>
                <p className="text-jatai-gray">{item.description}</p>
              </div>
            </div>
          ))}
        </div>
        
        <div className="relative py-16 px-8 bg-gradient-to-r from-jatai-purple to-jatai-blue rounded-3xl shadow-xl overflow-hidden reveal">
          ...
          <div className="relative z-10 text-center text-white">
            <h3 className="text-2xl md:text-3xl font-bold mb-4">Transforme sua operação empresarial</h3>
